...
# Obtener un trabajador y clínica existentes
try:
    ...
    print(f"[TEST] Trabajador: {trabajador.nombre}")
    ...
    if clinica_match:
        ...
        if 'Fecha' in clinica_section:
            ...
            if fecha_row:
                print(f"[TEST] Fila Fecha: {fecha_row.group(0)}\n")
        else:
        ...
        if 'Hora' in clinica_section:
            ...
            if hora_row:
                print(f"[TEST] Fila Hora: {hora_row.group(0)}\n")
        else:
        ...
        print(f"[TEST] === MOSTRANDO SECCIÓN DE CLÍNICA COMPLETA ===\n")
        print(clinica_section[:1500])
        print("\n[TEST] ...\n")
    else:
        print(f"[TEST] ✗ Sección de clínica NO encontrada\n")
    
    # Guardar HTML a archivo para inspección manual
    debug_dir = os.path.join(os.path.dirname(__file__), 'debug_emails')
